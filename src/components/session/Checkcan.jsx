...
export default function CheckCan() {
  ...
  useEffect(() => {
    ...
    if (!userlocal) {
      ...
      setSnackColor("error");
      setOpenSnack(true);
      setTimeout(() => navigate("/candidat/connexion"), 2500);
      return;
    }

    try {
      const userParsed = JSON.parse(userlocal);
      if (userParsed.rolelocal !== "candidat") {
        setSnackMessage("Accès réservé aux candidats !");
      ...
      setSnackMessage("veillez connecter en tant que candidat !");
      setSnackColor("error");
      setOpenSnack(true);
      setTimeout(() => navigate("/candidat/connexion"), 2500);
    }
